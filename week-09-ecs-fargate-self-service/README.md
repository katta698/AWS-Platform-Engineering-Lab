...
- **ALB does not rewrite paths — apps must be path-prefix-aware.** The full path (e.g. `/demo-nginx/`) is forwarded to the container unmodified; ALB has no "strip prefix" listener rule action (unlike NGINX Ingress, Traefik, or API Gateway). Every app deployed behind this platform needs to serve its content under `/<service_name>/`, not assume it owns `/`. The health check path (`elbv2.create_target_group`'s `HealthCheckPath`) is set to `/<service_name>/` to match — a hardcoded `/` will fail health checks the moment the app is actually path-aware, which is exactly what happened during live testing (2026-07-12): a task that was genuinely healthy got killed because the health check hit `/` while the app only served `/demo-nginx/`.
- **No NAT Gateway — and this rules out `public.ecr.aws` images.** Fargate tasks run in private subnets with zero internet route; they reach *private* ECR and CloudWatch Logs through VPC interface endpoints (plus an S3 gateway endpoint, since ECR image layers live in S3). But AWS's **Public** ECR Gallery (`public.ecr.aws`) is a completely different service with no VPC endpoint — pulling from it needs real internet access this design deliberately doesn't have. Confirmed via a real `CannotPullContainerError: ... dial tcp ... i/o timeout` during live testing. **Every ticket's `image_uri` must point at an image already in *private* ECR** (this service's own repo, or another private repo this account can reach) — not a public registry, unless a future week adds a NAT Gateway or a public ECR VPC endpoint doesn't exist to add.
- **This pipeline provisions services, it does not build images.** A ticket supplies an existing `image_uri` already pushed to a private ECR repo (the service's own repo, created by this pipeline, or pushed there separately). Building/pushing images is explicitly out of scope — that's Week 11's territory (CI/CD Pipeline).
