"""
EBS Savings API — Lambda handler
Runs the CUR Athena query + EC2 DescribeVolumes across accounts,
then returns structured JSON to the API Gateway.
"""
import json
import os
import time
...
MEMBER_ROLE_NAME = os.environ.get("MEMBER_ROLE_NAME", "")
...
def run_athena_query(sql: str) -> list[dict]:
    ...
    # Poll until complete (max 55 s — Lambda timeout is 60 s)
...
def get_ec2_client(account_id: str | None = None) -> boto3.client:
    """Return an EC2 client, optionally assuming a cross-account role."""
    if not account_id or not MEMBER_ROLE_NAME:
        return ec2
...
def describe_volumes(account_id: str | None = None) -> list[dict]:
...
def enrich_volumes(raw_volumes: list[dict]) -> list[dict]:
    ...
    for v in raw_volumes:
        ...
        if not attached:
            ...
            # We don't know instance power state here without DescribeInstances;
            # set to attached-running as a safe default — front-end shows "Review"
        ...
        # Monthly cost estimate: volume_size_gb * price_per_gb
        # Rough prices — real cost comes from Athena CUR join
        ...
        enriched.append({
            ...
            "instance_type":    None,  # would need DescribeInstances cross-ref
            "instance_state":   None,
            "monthly_cost":     round(v["Size"] * price, 2),
            "age_days":         age_d,
            "has_recent_snapshot": False,  # placeholder — DescribeSnapshots would fill this
            "account_id":       "",
            "account_name":     "",
...
def build_monthly_trend(rows: list[dict]) -> list[dict]:
    ...
    if not sorted_months:
        return []
    # Baseline = average of first 3 months
...
def build_accounts(rows: list[dict], cleanup_start: str) -> list[dict]:
    # Collect monthly spend per account, split by pre/post cleanup
...
def main(event, context):
    ...
    # 2. EC2 DescribeVolumes — Phase 2 only (requires MEMBER_ROLE_NAME to be set).
    #    Phase 1 (CUR-only) skips cross-account EC2 calls entirely.
    if MEMBER_ROLE_NAME:
        raw_volumes = describe_volumes()
        volumes     = enrich_volumes(raw_volumes)
    else:
        volumes = []
    ...
    # Find the month spend started dropping as cleanup_start_month
    cleanup_start = next(
        (t["month"] for t in trend if t["spend"] < baseline * 0.95),
        trend[-1]["month"] if trend else "",
    )

    # Phase 2: count real unattached volumes from EC2 inventory.
    # Phase 1: sum volume_count from CUR rows (proxy from billing data).
    if MEMBER_ROLE_NAME:
        volumes_deleted = len([v for v in volumes if v["attachment_state"] == "unattached"])
    else:
        volumes_deleted = sum(int(r.get("volume_count") or 0) for r in rows)
    ...
    payload = {
        ...
        "ou_savings":        [],   # requires AWS Organizations tagging
